...
#    feature = hog.HoG_output_vector()
    ...
    fn_dataset_cross = ["X_train_part", "X_test_part", "y_train_part", "y_test_part"]
    X_train = np.load(dir_save_feature + fn_dataset_cross[0] + ".npy")
    y_train = np.load(dir_save_feature + fn_dataset_cross[2] + ".npy")
    ...
    for i in range(len(c)):
        parameter = '-c ' + str(c[i]) + ' -v 5'
        
        para = util.parameter(parameter)
        prob = util.problem(y_train,X_train)
    
        CV_ACC = util.train(prob, para)
        print("liblinear -c " + str(c[i]) + " -v 5 acc = " + str(CV_ACC))
...
#        parameter = "-s 0" + " -t " + str(t) + " -c " + str(c[i]) + " -g " + str(g)
